...
function safe_glob($dir, $pattern) {
    $path = $dir . DIRECTORY_SEPARATOR . $pattern;

    if (preg_match('/\{(.+?)\}/', $pattern, $m)) {
        $result = [];
        foreach (explode(',', $m[1]) as $alt) {
            $altPattern = str_replace($m[0], $alt, $pattern);
            $result = array_merge($result, glob($dir . DIRECTORY_SEPARATOR . $altPattern));
        }
        return $result;
    }
    return glob($path);
...
function display_info_files(string $files_dir) {
    ...
    foreach ($patterns as $pattern) {
        $files = safe_glob($files_dir . DIRECTORY_SEPARATOR . $pattern, GLOB_BRACE);
